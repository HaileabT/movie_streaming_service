...
export const fetchWatchLater = async (stateSetter: Function, loadingSetter: Function, toastShow: Function, errorSetter?: Function) => {
  try {
    ...
    if (error.response?.status === 401) {
      errorSetter && errorSetter("Please sign in to view your watch later list");
...
export const fetchFavorites = async (stateSetter: Function, loadingSetter: Function, toastShow: Function, errorSetter?: Function) => {
  try {
    ...
    if (error.response?.status === 401) {
      errorSetter && errorSetter("Please sign in to view your favorites");
